...
class Prepare(object):
    ...
    def __init__(
        self, data_file, fp_size=255, buyable_file=None, retrain=False, path=None, name=None
    ):
        # parse file
        self.name = None
        self.path = path
    ...
    def _read(self, file):
        ...
        selected2 = selected.drop_duplicates(
            ["product", "category"], keep="first", inplace=False
        )  # 5083
        ...
        for i in tqdm(range(len(df))):
            ...
            for n in range(1, self.k + 1):
                if df.loc[i]["reactant" + str(n)] != "none":
                    file1[df.loc[i]["product"]][df.loc[i]["category"]].append(
                        df.loc[i]["reactant" + str(n)]
                    )
        ...
        def mol_to_fp(mol, FINGERPRINT_SIZE=16384, FP_rad=3):
            ...
            return np.array(
                AllChem.GetMorganFingerprintAsBitVect(
                    mol, FP_rad, nBits=FINGERPRINT_SIZE, useChirality=True, useFeatures=False
                ),
                dtype=np.int32,
            )
    ...
    def generate_ground_truth(self):
        ...
        if os.path.isfile(self.path + "ground_truth.npy"):
            ...
            for i in tqdm(range(len(target_product))):
                ...
                ground_truth[target_product[i]] = {
                    "cost": min(target1.cost),
                    "path": target1.temp[index],
                }
